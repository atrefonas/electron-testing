...
function startGoWebServer() {
  ...
  goServerProcess.stdout!.on('data', (data) => {
    console.log(`go-web-server stdout: ${data}`);
  });

  goServerProcess.stderr!.on('data', (data) => {
    console.error(`go-web-server stderr: ${data}`);
  });

  goServerProcess.on('close', (code) => {
    console.log(`go-web-server process exited with code ${code}`);
...
app
  ...
  .then(() => {
    createWindow();
    startGoWebServer(); // Start the Go server
    app.on('activate', () => {
